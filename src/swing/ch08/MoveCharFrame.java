...
//키 리스너를 구현하여 키보드 입력을 받아서 라벨을 움직이는 클래스 설계
public class MoveCharFrame extends JFrame implements KeyListener {
    ...
    private void initData() {
        ...
        label.setSize(100,100); // 라벨 자체 크기
        label.setLocation(INIT_X,INIT_Y); // 초기 시작 위치 (중앙 부근)
    }

    private void setInitLayout() {
        //중요!
        setLayout(null); // 좌표로 직접 움직이기 위해 기본 레이아웃을 제거한다
        add(label);
        setVisible(true);
    ...
    private void AddEventListener() {
        //1. 프레임에 키 리스너를  등록 (프레임 자체에 등록)
        this.addKeyListener(this); // 나 자신을 리스너로 임명 가능
    ...
    public void keyPressed(KeyEvent e) {
        ...
        int x = label.getX(); // 현재 자신의 x좌표값을 반환
        int y = label.getY(); // 현재 자신의 y좌표값을 반환
